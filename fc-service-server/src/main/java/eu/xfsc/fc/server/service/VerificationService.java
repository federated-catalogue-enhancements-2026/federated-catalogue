...
public class VerificationService implements VerificationApiDelegate {
  ...
  public ResponseEntity<VerificationResult> verify(Boolean verifySemantics,
    ...
    log.debug(
        "verify.enter; got body of length: {}; verify semantics: {}, vp-signature: {}, vc-signature: {}, framework: {}",
        body.length(), verifySemantics, verifyVPSignature, verifyVCSignature, framework);
    VerificationResult verificationResult = verificationService.verifyCredential(new ContentAccessorDirect(body),
            verifySemantics, verifyVPSignature, verifyVCSignature);
